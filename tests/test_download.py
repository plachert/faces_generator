...
def test_failing_download(mocked_set, cleanup):
    mocked_set.side_effect = lambda: FailingSet()
    try:
        download.run(cleanup, 3)
    except SimulatedException:
        pkl_files = list(cleanup.glob("*.pkl"))
        assert len(pkl_files) == 1
        assert pkl_files[0].name == "seen_images.pkl"


def test_successful_download(cleanup):
    download.run(cleanup, 3)
    jpg_files = list(cleanup.glob("*.jpg"))
    pkl_files = list(cleanup.glob("*.pkl"))
...
def test_incremental_download(cleanup):
    download.run(cleanup, 3)
    download.run(cleanup, 3)
    jpg_files = list(cleanup.glob("*.jpg"))
    assert len(jpg_files) == 6
    with open(cleanup / "seen_images.pkl", "rb") as f:
        seen = pickle.load(f)
    assert len(seen) == 6
